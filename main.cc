...
class C {
 private:
  int taskCount_ = 0;
 public:
  explicit C(int taskCount) : taskCount_(taskCount) {}
...
class InputTask : public AbstractTask<double, int, float> {
 ...
 public:
  ...
  ~InputTask() override {
    totalCount_ += this->count;
//    std::cout << "I did " << this->count << " work, Total count = " << totalCount_ << std::endl;
  }
