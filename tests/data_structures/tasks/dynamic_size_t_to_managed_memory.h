...
class DynamicSizeTToManagedMemory : public hh::AbstractTask<1, size_t, DynamicManagedMemory> {
 public:
  ...
  void execute(std::shared_ptr<size_t> data) override {
    auto memory = std::dynamic_pointer_cast<DynamicManagedMemory>(this->getManagedMemory());
    memory->alloc(*data);
    this->addResult(memory);
  }

  std::shared_ptr<hh::AbstractTask<1, size_t, DynamicManagedMemory>> copy() override {
    return std::make_shared<DynamicSizeTToManagedMemory>(this->numberThreads());
  }
};
