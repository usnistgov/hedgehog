...
class DefaultSlot : public ImplementorSlot {
 private:
  std::unique_ptr<std::set<abstraction::NotifierAbstraction *>> const
      notifiers_ = nullptr; ///< List of notifiers linked to this slot

 public:
  /// @brief Default constructor
  explicit DefaultSlot()
  ...
  [[nodiscard]] bool hasNotifierConnected() const override { return !notifiers_->empty(); }

  /// @brief Add a notifier to the list of connected notifiers
  /// @param notifier Notifier to add to the list of connected notifiers
  void addNotifier(abstraction::NotifierAbstraction *const notifier) override { notifiers_->insert(notifier); }
  ...
  void removeNotifier(abstraction::NotifierAbstraction *const notifier) override { notifiers_->erase(notifier); }
};
}
}
}
