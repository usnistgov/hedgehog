...
///
/// @attention Data that is managed by an AbstractMemoryManager, requires:
...
///
/// The amount of data available is set at construction: AbstractMemoryManager(size_t const &capacity). At construction,
...
/// AbstractMemoryManager::getManagedMemory(). If no data is available (i.e. the Pool is empty), the task will
...
/// is returned to the AbstractMemoryManager multiple times before being recycled and sent back to the Pool.
/// -# MemoryData::canBeRecycled(): Boolean method to determine if the MemoryData can be recycled, and send back to the
/// Pool.
/// -# MemoryData::recycle(): Recycle the MemoryData. The data given by the AbstractMemoryManager is default constructed
...
///
/// The only pure virtual method is the copy method to duplicate a derived AbstractMemoryManager to different
...
/// AbstractMemoryManager::copy <br>
/// AbstractMemoryManager::initialize (advanced, should be used carefully) <br>
/// AbstractMemoryManager::initializeMemoryManager (user-defined customization)
///
/// @tparam ManagedMemory Type of data that will be managed by an AbstractMemoryManager
...
class AbstractMemoryManager {
 ...
 public:
  ...
  virtual std::shared_ptr<AbstractMemoryManager<ManagedMemory>> copy() = 0;
...
/// @brief Definition of AbstractMemoryManager, for data that do not derive from MemoryData and/ or are not default
/// constructible
